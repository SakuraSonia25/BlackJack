...
public class Round {
    ...
    /* TURN METHODS */
    // Player Turn - idk how to describe this method
    public void playerTurn(Player player) {
        ...
        if (player instanceof BotPlayer) {
            ...
            } else if (turnOutcome != 'n' && choice == 1) {
                System.out.println("You got a LUCKYHAND!"); 
            }
    ...
    /* ACTION METHODS */
    // person hits a card, return if the hand burst, hv a lucky card or is still below 21
    ...
    // player stand, this method auto hits a card with hand is <16, and return if the hand burst, hv a lucky card or is still below 21 
    ...
    // human dealer is challenging, if hand is <16, this method auto hits a card, after that player selects oppenents, then challenges the oppenents
    public int dealerChallenge(){
        ...
        while (dealerHand.getHandScore() < 16) { 
            ...
            System.out.println("-- your current handscore is " + dealerHand.getHandScore() + "(" + (getPlayerLucky(this.dealer) == 'n' ? "": luckyHandMap.get(getPlayerLucky(this.dealer))) + ")");
    ...
    // bot dealer is challenging, if hand is <16, this method auto hits a card,after that, allow bot choose oppenents and challenge till all players are challlenged
    ...
    // this method is called to prompt human dealer to choose oppenents
    public List<Player> chooseOpponents() {
        ...
        while (stillSelecting) {
            ...
            // continue selecting
            // invalid choice 
            else if (((choice - 1) < 0 || (choice - 1) >= this.players.size()) ||  getPlayerResult(this.players.get(choice -1)) != 'c' ) {
                // err msg
    ...
    // update all the cash amt of players in the specified player list
    ...
    public void displayChallengeResult(List<Player> oppenents) {
        ...
        System.out.println("current cash amount: " + this.dealer.getCashAmt());
        promptEnterKey(); // prompt player to press enter key to proceed
    }

    /* HELPER METHODS */
    // Check for BJ // {true - hv, false - no hv}
